...
{
	return error_log(__FUNCTION__.'() '.__FILE__.':'.__LINE__.': '.$msg);
}
...
class AA_DEBUG
{
	 ...
	 */
	 var $actions = array(
		...
		'disable'		=>array('title'=>'Quick Disable', 'nonce'=>''),
		'enable'		=>array('title'=>'Quick Enable', 'nonce'=>'')
	 );
	...
	function AdminBar($wp_admin_bar)
	{
		...
		if($this->options["admin_footer"]=='1'||$this->options["wp_footer"]=='1') $actions['disable']=$this->actions['disable'];
		else $actions['enable']=$this->actions['enable'];
	...
	function Load()
	{
		...
		if('GET'== $_SERVER['REQUEST_METHOD']) $this->HandleGet();
	...
	function HandleGet()
	{
		...
		if( !$this->check_auth() ) wp_die( '<strong>ERROR</strong>: User does not have permission to manage options. '.__FUNCTION__.':'.__LINE__ );
		...
		if( isset($_GET["{$this->_plugin['op']}_action"], $_GET['_wpnonce']) && $_GET["{$this->_plugin['op']}_action"]=='files' && isset($_GET['file']) ) 
		{
			...
			if(is_dir($f)) {
				...
				//ISCLOG::epx(array('get'=>$_GET['file'], 'dec'=>$f));
			} else {
				//ISCLOG::epx("_wpnonce set but {$f} not a dir");
				if(is_file($f)) $this->options['dirtoexplore']=rtrim($f,'/');
			}

		} else {
			//ISCLOG::epx("_wpnonce nonce not set");
			return;
		}
	...
	function HandlePost()
	{
		...
		elseif(isset($_POST["{$this->_plugin['op']}_{$this->_plugin['nonce']}_reset"])) {
			...
			//ISCLOG::epx("{$this->_plugin['op']}_{$this->_plugin['nonce']}  nonce not set");
			return;
		}
		...
		if (isset($_POST["{$op}_save_debug_options"]))
		{
			...
			foreach(array('plugin_debug_level','display_height', 'error_reporting') as $k) 
				if(isset($_POST["{$op}_{$k}"]) && !empty($_POST["{$op}_{$k}"])) $this->options["{$k}"] = absint($_POST["{$op}_{$k}"]);
			//------------------- absint
	...
	function AdminPage()
	{
		...
		switch($this->options['page'])
		{
			...
			case 'files':
				...
				echo '<hr />';
				$this->ff( array('form'=>6,'type'=>'hidden','id'=>$this->_plugin['op'].'_'.$this->_plugin['nonce'],'name'=>$this->_plugin['op'].'_'.$this->_plugin['nonce'],'value'=>wp_create_nonce($this->_plugin['nonce']),'pre'=>'<p style="display:none;">','post'=>'') );
				$this->ff( array('form'=>6,'type'=>'hidden','id'=>'_wp_http_referer','name'=>'_wp_http_referer','value'=>(esc_attr($_SERVER['REQUEST_URI'])) ,'pre'=>'','post'=>'</p>') );
				$this->ff( array('form'=>6,'type'=>'hidden','id'=>$this->_plugin['op'].'_save_debug_options','name'=>$this->_plugin['op'].'_save_debug_options','value'=>'save_debug_options' ,'pre'=>'','post'=>'</p>') );
				echo '<div id="'.$this->_plugin['op'].'">';
				$this->ff( array('form'=>2,'type'=>'text','class'=>'aa_wide','title'=>'Dir/File to Explore: ','id'=>'dirtoexplore','value'=>$this->options['dirtoexplore']) );
				echo '</div>';
				echo '</form>';
			...
			case 'wpconfig':
				...
				echo '<p>This is just a recommendation, this is not editable. Add this to your wp-config.php at the bottom BEFORE the wp-settings is included.  This is unneccessary if you can modify your php.ini - See my <a href="http://www.askapache.com/wordpress/advanced-wp-config-php-tweaks.html">wp-config.php tutorial</a>.</p>';
				...
				//echo preg_replace('#color="(.*?)"#', 'style="color:\\1"', str_replace(array('<font ', '</font>'), array('<span ', '</span>'), highlight_string(stripslashes($rec), true)));
			break;
			...
			case 'home':
				...
				echo '<div id="'.$this->_plugin['op'].'">';
				$this->ff( array('form'=>1,'type'=>'checkbox','title'=>'<strong>View in admin_footer</strong>','id'=>'admin_footer','checked'=>($this->options['admin_footer']=='1'),'value'=>$this->options['admin_footer']) );
				$this->ff( array('form'=>1,'type'=>'checkbox','title'=>'<strong>View in wp_footer</strong>','id'=>'wp_footer','checked'=>($this->options['wp_footer']=='1'),'value'=>$this->options['wp_footer']) );
				$this->ff( array('form'=>1,'type'=>'checkbox','title'=>'<strong>Log Errors to File</strong>','id'=>'log_errors','checked'=>($this->options['log_errors']=='1'),'value'=>$this->options['log_errors']) );
				...
				echo '<hr />';
				$this->ff( array('form'=>2,'type'=>'text','class'=>'aa_mid','title'=>'Error Log File','id'=>'logfile','value'=>$this->options['logfile']) );
				echo '<hr />';
				...
				$this->ff( array('form'=>2,'type'=>'text','class'=>'aa_small','title'=>'Plugin Debug Level (0-100)','id'=>'plugin_debug_level','value'=>$this->options['plugin_debug_level']) );
				$this->ff( array('form'=>2,'type'=>'text','class'=>'aa_small','title'=>'Output Display Height (0-5000)','id'=>'display_height','value'=>$this->options['display_height']) );
				echo '<hr />';
				...
				$this->ff( array('form'=>1,'type'=>'checkbox','title'=>'<strong>Verbose Module Output</strong>','id'=>'verbose_modules','checked'=>($this->options['verbose_modules']=='1'),'value'=>$this->options['verbose_modules']) );
				echo '<hr />';
				//$this->ff( array('form'=>5,'type'=>'hidden','id'=>'debug_mods_v','name'=>'debug_mods_v','value'=>$this->options['debug_mods_v'],'pre'=>'','post'=>'') );
				$this->ff( array('form'=>5,'type'=>'hidden','id'=>'debug_mods','name'=>'debug_mods','value'=>$this->options['debug_mods'],'pre'=>'','post'=>'') );

				foreach ($this->debug_mods as $id => $info) {
					$this->ff(array('form'=>3,'type'=>'checkbox','title'=>$info[1],'id'=>'debug_mods','name'=>'debug_mods[]','value'=>$id,'checked'=>(($id & $this->options['debug_mods'])==$id),'pre'=>"<p class='c4r'>",'post'=>"</p>")) ;
				...
				$this->ff( array('form'=>2,'type'=>'text','class'=>'aa_small','title'=>'PHP Error Reporting Level (try 2147483647)','id'=>'error_reporting','value'=>$this->options['error_reporting']) );
				...
				$this->ff( array('form'=>5,'type'=>'submit','class'=>'button-primary','id'=>'save_debug_options','name'=>'save_debug_options','value'=>'Save Changes &raquo;','pre'=>'<p class="submit">','post'=>' '.str_repeat('&nbsp;',11).' ') );
				$this->ff( array('form'=>5,'type'=>'submit','class'=>'button-secondary','id'=>'action_reset','name'=>'action_reset','value'=>'Revert to Defaults &raquo;','pre'=>'','post'=>'</p>') );
	...
	*/
	function display_navigation_menu()
	{
		...
		foreach ($this->pages as $id => $idv) {
			printf('<li><a href="%1$s" title="%3$s" class="aa_css_menu_btn">%2$s</a></li>', $this->pages[$id]['nonce'], $this->pages[$id]['name'], $this->pages[$id]['title']);
		}
		
		echo '<li><a class="aa_css_menu_btn" title="WP Plugin Home" href="http://wordpress.org/extend/plugins/askapache-debug-viewer/">WP PLUGIN HOME</a></li></ul>';
	...
	*/
	function display_ini_error_configs()
	{
		...
		// SHOW ERROR INFO
		$e=array();$eany=0;
		foreach($this->get_error_levels(0,'defined') as $k=>$v) $eany |= $e["$k"]=constant($k);
		/*
		...
		*/
		...
		foreach ($directives as $k)
		{
			$v1=@ini_get($k);
			$v2=@get_cfg_var($k);
			if(empty($v1) && empty($v2))continue;
			if($v1 != $v2 && ! empty($v2) ) $v2=' | <tt>'.$v2.'</tt>';
			else $v2='';
	...
	function footer_output()
	{
		...
		if( is_admin() && $this->options['admin_footer']!='1') {
			aadv_error_log(__FUNCTION__.':'.__LINE__.' is_admin and admin_footer option is off, ending footer_output');
			return;
		}

		if(!is_admin() && $this->options['wp_footer']!='1') {
			aadv_error_log(__FUNCTION__.':'.__LINE__.' !is_admin and wp_footer option is off, ending footer_output');
			return;
		}
		...
		echo '</div><!--aao-->  ';
		if(!is_admin()) echo '<a id="aaslink" href="'.admin_url($this->_plugin['action']).'">Settings</a> ';
		echo '</div><!--aan-->';
	...
	function live_debug()
	{
		...
		*/
		...
		//if ( defined('WP_DEBUG_LOG') && WP_DEBUG_LOG ) ini_set( 'log_errors', 1 );
	...
	function ff($args)
	{
		...
		if(empty($args['name']))$args['name']=$args['id'];
	...
	*/
	function get_debug_wordpress($vb=false)
	{
		...
		global $wp_query, $wp, $wpdb, $wp_rewrite, $wp_object_cache, $user_email, $user_ID, $wp_admin_bar, $post, $template, $post_ID, $wp_the_query, $user, $userdata, $current_user, $current_site, $current_blog, $current_screen, $wp_roles, $wp_user_roles, $merged_filters, $wp_filter, $wp_actions, $wp_scripts, $wp_styles, $wp_taxonomies;
	...
	*/
	...
	{		
		//if($vb==false)return var_export(array_diff(get_defined_vars(), array(array())));
	...
	*/
	function get_debug_globalvars($vb=false)
	{
		...
		foreach($globalkeys as $k=>$v) {
			$val=$GLOBALS[$v];
			$gtype=gettype($val);
			if($gtype=='NULL'|| ( in_array($gtype,array('string','float','double','integer','array')) && empty($val)) ) continue;
			$out='$'.$v.' ('.$gtype.(in_array($gtype,array('string','array'))?'':') ');
			//ISCLOG::epd(array($gtype));
			
			switch($gtype)
			{
				case 'float':
				case 'double':
				case 'integer': $out.='= '.htmlspecialchars($val); break;
				
				case 'boolean': $out.='= '.($val===true ? 'true' : 'false'); break;
				
				case 'resource': $out.='= '.get_resource_type($val); break;

				case 'string': $out.='#'.strlen($val).') = '.htmlspecialchars($val); break;
				case 'array':
					//$n6=substr( preg_replace('/\s\s+/', ' ', str_replace(' => ','=>',  implode('', explode("\n",print_r($val,1),30) ) ) ) ,5,50);
					$out.='#'.sizeof($val).') => ';
				break;
	...
	*/
	function get_debug_queries($vb=false)
	{
		...
		$mesg = (!defined('SAVEQUERIES') || !SAVEQUERIES) ? '<p><code>define(\'SAVEQUERIES\', true);</code>' . __('in your') . ' <code>wp-config.php</code></p>' : '';
	...
	*/
	function get_debug_sockets($vb=false)
	{
		...
		if (false === ( $fp = fsockopen($_SERVER['HTTP_HOST'], $_SERVER['SERVER_PORT'], $errno, $errstr,5) ) || !is_resource($fp) ) echo $this->socket_error($fp, (int)$errno, $errstr);
		else {
			$response='';
			$request=join($LF,array(
								 'GET / HTTP/1.0',
			...
			if(is_resource($fp)) echo $this->_sockdebug($fp);
	...
	function get_debug_extensions( $vb = false )
	{
		...
		if($vb)
		{
			foreach ( (array )@get_loaded_extensions() as $v )
			{
				...
				foreach((array)$ext->getINIEntries() as $kk=>$vv)$ret[$v]['inis'][$kk]=$vv;
	...
	function get_debug_posix( $vb = false )
	{
		...
		foreach ($posix_vars as $t=>$fn) if($this->_cf($fn))$oa[$t]=$fn();
	...
	function get_debug_classes( $vb = false )
	{
		...
		foreach ($classes as $k)
		{
			...
			{
				if( $this->_cf('get_class_methods') && $this->_cf('get_class_vars') )
				{
	...
	function get_debug_defined( $vb = false )
	{
		...
		if(!$vb) $constants=array_slice($constants, ($pos1-10));
		
		foreach ($constants as $k=>$v) $oa[$k]=$v;
	...
	function get_debug_inis( $vb = false )
	{
		...
		if ($vb!==false && $this->_cf('ini_get_all')) {
		...
		if($this->_cf('phpinfo'))
		{
			...
			if($type!=0)
			{
				$oa = preg_replace(array('#^.*<body>(.*)</body>.*$#ms','#<h2>PHP License</h2>.*$#ms','#<h1>Configuration</h1>#',"#\r?\n#","#</(h1|h2|h3|tr)>#",'# +<#',"#[ \t]+#",
									  '#&nbsp;#','#  +#','# class=".*?"#','%&#039;%','#<tr>(?:.*?)" src="(?:.*?)=(.*?)" alt="PHP Logo" /></a>' . '<h1>PHP Version (.*?)</h1>(?:\n+?)</td></tr>#',
									  '#<h1><a href="(?:.*?)?=(.*?)">PHP Credits</a></h1>#','#<tr>(?:.*?)" src="(?:.*?)=(.*?)"(?:.*?)Zend Engine (.*?),(?:.*?)</tr>#',"#	+#",'#<tr>#','#</tr>#'
									  ),
			...
			ob_get_clean());
				...
				foreach ( $sections as $s )
				{
					...
					foreach ($askapache as $m) $oa[(substr( $s, 0, strpos($s, '</h2>') ))][$m[1]]=( !isset($m[3]) || $m[2]==$m[3] ) ? (isset($m[2]) ? $m[2] : '') : array_slice($m,2);
				}
			} else {
	...
	function get_debug_footerhelper($vb=false)
	{
		...
		global $wp_query, $wp, $wp_the_query, $user_email, $wp_admin_bar, $wpdb, $post, $post_ID, $user, $current_user, $_wp_theme_features, $template, $current_screen, $hook_suffix, $wp_importers;
		...
		if(is_object($wp_the_query) && is_array($wp_the_query->query)){
			foreach($wp_the_query->query as $k=>$v) {
				if(!empty($v) && strlen($v)>0) $out['q']['wp_the_query->query'][$k]=$v;
			}
		}
		
		if(is_object($wp) && is_array($wp->query_vars)){
			foreach($wp->query_vars as $k=>$v) {
				if(!empty($v) && strlen($v)>0) $out['q']['wp->query_vars'][$k]=$v;
			}
		}
		...
		if(is_admin()){
			...
			if(is_singular()||$hook_suffix=='post.php'){
				$d=has_meta (get_queried_object_id());$out['has_meta']=$d;
				$d=get_post_custom (get_queried_object_id());$out['get_post_custom']=$d;
				$d=get_meta_keys();$out['get_meta_keys']=$d;
			}
	
	
			if($hook_suffix=='post.php'){
				$d=$post; $out['POST']=$d; $d=get_the_category( $post->ID); $out['get_the_category-id']=$d;
			}

		} else {
			
			$out['object']=$object = get_queried_object(); 
			if(isset($out['object']->post_content)) $out['object']->post_content='cleared by aa_debug for clean output';
			$out['ID']=$object_id = (int)get_queried_object_id(); 
			
				
			if(is_singular())
			{				
				$object_status=get_post_status($object_id);;
				if($object_status !== false && !empty($object_status) && !is_null($object_status)) $out['status']=$object_status;
				
				$object_type=get_post_type($object_id);
				if($object_type !== false && !empty($object_type) && !is_null($object_type)) $out['type']=$object_type;
				
				$object_mime_type=get_post_mime_type($object_id);
				if($object_mime_type !== false && !empty($object_mime_type) && !is_null($object_mime_type)) $out['mime_type']=$object_mime_type;

				$object_format=get_post_format($object);
				if($object_format !== false && !empty($object_format) && !is_null($object_format)) $out['format']=$object_format;
			}
			
	
			$d=get_theme_data(STYLESHEETPATH . '/style.css');$out['get_theme_data']=$d;
			$d=get_template();$out['get_template']=$d;
			$d=get_page_templates(); $out['get_page_templates']=$d;
		...
		$out['UPLOAD']=(($this->_cf('wp_upload_dir')) ? wp_upload_dir() : array());
	...
	function get_debug_options($vb=false)
	{
		...
		$oa=array();
		$oa=wp_cache_get( 'alloptions', 'options' );
	...
	function get_debug_templates($vb=false)
	{
		...
		global $wp_query, $user_email, $wp_admin_bar, $wpdb, $post, $post_ID, $user, $current_user, $_wp_theme_features, $template, $current_screen, $hook_suffix, $wp_importers;
	...
	function get_debug_filters( $vb = false )
	{
		...
		$ret=array('wp_filters'=>array_keys($wp_filter), 'wp_actions'=>$wp_actions, 'wp_filter'=>$wp_filter, ' '=>"\n\n\n\n\n\n\n\n");
	...
	function get_debug_scripts( $vb = false )
	{
		...
		$g=$wp_scripts;
		if(!$vb)$g->registered=array_keys($g->registered);
	...
	*/
	...
	{
		static $links=null;
		if(is_null($links))$links=array();

		if(!empty($title) && !in_array($title,$links)) $links[]=$title;
		
		if($print){
			$out='';
			foreach($links as $k) $out.="<li><a href='#tabs-".sanitize_title_with_dashes($k)."'>{$k}</a></li>\n";
			return "\n<ul>".$out."</ul>\n";
		}
	}
	 ...
	 * @param mixed $vb
	 */
	function get_socket_request($n='',$p='')
	{
		$this->t(__FILE__,__CLASS__,__FUNCTION__,__LINE__,'',5);
		...
		if($ret=='') {
			...
			if (false === ( $fp = fsockopen($_SERVER['HTTP_HOST'], 80, $errno, $errstr,5) ) || !is_resource($fp) ) echo $this->socket_error($fp, (int)$errno, $errstr);
				...
				if(strpos($headers,'200 OK')!==false) $ret='<pre class="aa_pre1">'.htmlspecialchars($body).'</pre>';
				else {
					ob_start();
					echo '<pre class="aa_pre1">'."\nFILE: {$f}\n". $this->_statls($f,1).'</pre>';
					echo "\n<h3>HEADER TRACE</h3><pre class='aa_pre1'>\n".str_repeat("&gt;",100)."\n".htmlspecialchars($request)."\n".str_repeat("&lt;",100)."\n".htmlspecialchars($headers)."</pre>";
					echo "<pre class='aa_pre1'>".htmlspecialchars($body)."</pre>";
					$ret=ob_get_clean();
				}
	...
	*/
	...
	function activate_ff_htaccess()
	{
		...
		if(!file_exists($htaccess_file)) echo "\nCANNOT CONTINUE: {$htaccess_file} not found.\n";
			...
			if(!file_put_contents($htaccess_file, $active_htaccess_rules))echo "\nCANNOT CONTINUE: {$htaccess_file} not written.\n";
		}
	...
	function deactivate_ff_htaccess()
	{
		...
		if(!file_exists($htaccess_file)) echo "\nCANNOT CONTINUE: {$htaccess_file} not found.\n";
			...
			if(!file_put_contents($htaccess_file, $inactive_htaccess_rules))echo "\nCANNOT CONTINUE: {$htaccess_file} not written.\n";
		}
	...
	*/
	...
	{
		if( $this->d($d) === false ) return;// aadv_error_log("t Skipped.. {$fu} {$this->_debug} <= {$d}");
	...
	function l($msg,$d=5,$b=false)
	{
		...
		if( $this->options && $this->options['log_errors']=='1' ){
			if($b){
				$t2=$this->tt($f);
				aadv_error_log("PHP Notice:  ".$f." [".$t2[0]."] [".$t2[1]."] [".$this->mem_usage(1)."] ".__CLASS__."::l()"." {$msg}");
			} else {
				if(empty($msg))return;
				aadv_error_log($msg);
			}
	...
	*/
	...
	function get_posix_info( $type = 'all', $id = '', $item = false )
	{
		...
		if(gettype($id)=='string' || $id=='')$id=$euid;
		$info = array();
		switch ( $type ):
		case 'group':  $info = ($this->_cf('posix_getgrgid') ? posix_getgrgid( $id ):'');  break;
		case 'user':  $info = ($this->_cf('posix_getpwuid') ? posix_getpwuid( $id ):'');  break;
		endswitch;
	...
	function check_auth()
	{
		...
		if ( !$current_user = wp_get_current_user() ) {
			$ret=false;
			aadv_error_log(__FUNCTION__.':'.__LINE__.' user not = wp_get_current_user');
		}

		if ( !current_user_can('manage_options')) {
			$ret=false;
			aadv_error_log(__FUNCTION__.':'.__LINE__.' current_user_cannot administrator');
		}
	...
	function _stat( $fl )
	{
		...
		$s = $ss = array();
		if ( ($ss = stat($fl)) === false ) return $this->l(__FILE__,__CLASS__,__FUNCTION__,__LINE__, "Couldnt stat {$fl}");
...
add_action( 'init', array(&$AA_DEBUG, 'Init'),10);
