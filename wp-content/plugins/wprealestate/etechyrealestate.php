...
Plugin URI: http://andytanoko.wordpress.com

Description: Real estate agents and property owners who wish to list and sell properties online.

Author: Etechy Solutions, modified by Andy Tanoko

Version: 2.8

Author URI: http://andytanoko.wordpress.com
...
add_action( 'init', 'wp_property_custom_init' );
 ...
 /* Andy Tanoko additional taxonomies*/

require_once('widget_area.php');

require_once('widget_advancedsearch.php');
...
// create two taxonomies, neighbourhoods and writers for the post type "property"
...
function create_property_taxonomies() {
	...
	register_taxonomy( 'neighbourhood', array( 'property' ), $args );
...
<div class="AdmfrmFld">
  ...
  <select id="et_er_adtype" name="et_er_adtype" class="AdmFrmList">
    ...
    <option <?php if ($et_er_adtype == 'Sold') {?> selected="selected"<?php }?>>Sold</option>

    <option <?php if ($et_er_adtype == 'Rented') {?> selected="selected"<?php }?>>Rented</option>
...
function tax_property_type()  {
	...
	register_taxonomy( 'propertytype', array('property'), $args );
